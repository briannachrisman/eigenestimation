...
def main(args, timer):
    ...
    eval_dataset = X_eval
    # Single model
    tms_model = AutoencoderSymmetric(n_features, n_hidden).to(args.device_id)
    model_checkpoint = torch.load(args.model_path, map_location=f"cuda:{args.device_id}")
    tms_model.load_state_dict(model_checkpoint["model"])
    ...
    eigenmodel = EigenModel(tms_model_p, model0, MSELoss(), args.n_eigenfeatures, args.n_eigenrank)
    
    
    # Initialize the trainer and start training
    trainer = Trainer(eigenmodel, train_dataset, eval_dataset, eigenmodel.compute_jacobian, args, timer)
    trainer.train()
