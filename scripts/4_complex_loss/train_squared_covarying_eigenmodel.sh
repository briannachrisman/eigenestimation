...
WANDB_PROJECT="squared-network-eigenmodel-covarying"
MODEL_PATH="outputs/toy_models/squared.pt"
CHECKPOINT_PATH="outputs/eigenmodels/squared_covarying.pt"
...
# Run training
torchrun --nnodes=$NNODES --nproc-per-node=$N_PROC $current_dir/train_squared_covarying_eigenmodel.py \
    --epochs $EPOCHS \
    --lr $LEARNING_RATE \
